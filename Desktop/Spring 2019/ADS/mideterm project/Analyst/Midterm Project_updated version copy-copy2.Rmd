...
```{r source_files}
training.data.file <- "MNIST-fashion training set-49.csv"
testing.data.file <- "MNIST-fashion testing set-49.csv"
```
...
sum(is.na(test.dat))
#check numeric
sum(is.numeric(as.matrix(train.dat[,2:50]))==F)
sum(is.numeric(as.matrix(test.dat[,2:50]))==F)
```
...
model.classification_tree<-function(TrainSet,TestSet){
  library(rpart)
  sample.size <- TrainSet[,.N]
  test <-as.matrix(TestSet[,2:50])
  start.time<-Sys.time()#record the start time
  #Fit classification tree model
  tree.model<-rpart(label~.,data=TrainSet)
  tree.model.prune<-prune(tree.model,cp=0.01)
  #predict using testset
  predTest<-predict(tree.model.prune, newx=test, type="class")
  end.time<-Sys.time()#record the end time
...
```{r load_model7}
# get result table
ClassificationTree.result <- iteration.samples.each.model("model.classification_tree")
# show as HTML table
...
model.lasso_regression<-function(TrainSet,TestSet){
  library(glmnet)
  sample.size <- TrainSet[,.N]
  X <-as.matrix(TrainSet[,2:50])
  y <-as.matrix(TrainSet[,1])
...
```{r load_model8}
# get result table
LassoRegression.result <- iteration.samples.each.model("model.lasso_regression")
# show as HTML table
...
model.neural_network<-function(TrainSet,TestSet){
  library(nnet)
  sample.size <- TrainSet[,.N]
  start.time<-Sys.time()
  #Fit the model
...
```{r load_model9}
# get result table
NeuralNet.result <- iteration.samples.each.model("model.neural_network")
# show as HTML table
